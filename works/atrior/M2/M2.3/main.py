...
# 处理用户缓存
# 使用cachetools创建自带过期时间的字典 maxsize ttl
UserInfoMap=TTLCache(maxsize=100, ttl=15)
UserRatingMap=TTLCache(maxsize=100, ttl=15)
...
def rating(ulist):
    ...
    arr=[]
    for i in userlist:
        if i in UserRatingMap:
            arr.append(UserRatingMap[i])
        else:
            UserRatingMap[i]=get_usr_rating(i)
            arr.append(UserRatingMap[i])
