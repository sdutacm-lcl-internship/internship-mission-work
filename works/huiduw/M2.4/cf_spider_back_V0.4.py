...
@cache.cached(timeout=15)  # 设置缓存时间为15s
...
def clearCache():
    try:
        ...
        cache_type = data.get('cache_type') #获取缓存类型
        handles = data.get('handles') #获取句柄列表

        if cache_type and handles:
            for handle in handles:
                cache.delete_memoized(get_user_info, handle)
                cache.delete_memoized(get_user_rating, handle)
        elif cache_type:
            cache.clear()
        else:
        ...
        return jsonify({"success": False, "message": "Error occurred while clearing cache"})
