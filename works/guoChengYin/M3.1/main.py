  ...
  for name in names:
    try:
      ...
      #缓存中没有该用户，重新爬取并保存数据
        ...
        request_info = crawler.crawl('https://codeforc.es/api/user.info?handles={}'.format(name) )
  ...
  try:
    ...
    if request_results['status'] == 200:
      ...
      for item in result:
        ...
        myUtils.data_save('data-user-rating.txt', {"handle": handle, "info": response_data})
      return jsonify(response_data),200
        # 查询不到 400 返回404
